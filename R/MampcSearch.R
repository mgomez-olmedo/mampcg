...
MampcSearch <- R6Class(
  ...
  public=list(
    # class contructor
    initialize=function(net, data, pc=FALSE, alpha, debug=FALSE){
      # sets debug flag
      private$debug <- debug
      ...
      private$graph <- empty.graph(names(data))
      ...
      for(i in 1:length(combinations[1,])){
        ...
        # add the edge
        private$graph <- set.edge(private$graph,from=combinations[1,i],to=combinations[2,i],
    ...
    compareStructure = function(){
      ...
      sk <- empty.graph(nodes(private$net))
      amat(sk,ignore.cycles=TRUE) <- amat(private$net)
      trueDependenceGraph <- skeleton(sk)
      
      # gets th skeleton of the current graph
      sk <- empty.graph(nodes(private$net))
      amat(sk,ignore.cycles=TRUE) <- amat(private$graph)
      currentDependenceGraph <- skeleton(sk)
      ...
      trueVsdf <- as.data.frame(vstructs(as.bn(modelstring(private$net))), stringsAsFactors=FALSE)
      # adds a new column with the type
      trueVsdf[,"type"] <- c(rep(1,nrow(trueVsdf)))
      if (private$debug){
        cat("V-structures in true net: \n")
        print(trueVsdf)
        cat("---------------------------------------------\n")
      }
            
      # gets th vstructures of the learnt model
      currentVsdf <- private$getModelVStructuresDataFrame()
      if (private$debug){
    ...
    # sets edgesInfo information. This is used for testing
    setEdgesInfo = function(edges){
      private$edgesInfo <- edges
    },
    
    # sets graph data member. This is used for testing
    ...
    # tests the learn algorithm without making independence tests
    testLearn = function(){
      if (private$debug){
  ...
  private = list(
    ...
    initialLoop=function(){
      ...
      while(l <= (nnodes-2)){
        ...
        pairs <- combn(nodes(private$graph),2)
        # forms the pairs of nodes
        pairs <- combn(nodes(private$graph),2)
    ...
    # private method for making the operations with a pair
    ...
    makeTests=function(pair,l,set){
      ...
      for(i in 1:length(sets)){
        ...
        # makes the test
        testResult <- ci.test(x=pair[[1]], y=pair[[2]],z=separator,private$data,test="mi")
    ...
    removeEdge=function(pair){
      ...
      private$graph <- drop.edge(private$graph,pair[[1]],pair[[2]])
    ...
    applyRule1 = function(){
      ...
      pairs <- private$matchEdgesWithCommonNode()
    ...
    # matches edges with a common node
    matchEdgesWithCommonNode = function(){
      # make cartesian product
      prod <- merge(private$edgesInfo,private$edgesInfo,all=TRUE,by=NULL)
    ...
    # generate a data frame from the list of lists with alignment pairs
    generateAlignmentDataFrame = function(pairs){
      ...
      if (private$containsData(pairs)){
         ...
         result <-rbindlist(candidatePairs)
      }
    ...
    # check the conditions for a pair in order to apply independence 
    # tests with it
    ...
    checkConditionForStep15 = function(pairInfo){
      ...
      if (nrow(edges) != 0){
        ...
        # change edge info: first edge remains and the second becomes to
        # block-wildcard
        edge <- changeEdgeInfo(pairInfo, 1, edgesInfo, "none", "none", debug)
      }
    ...
    # get for edge between nodeA and nodeB in the set of edges
    # passed as argument
    ...
    # gets the edges related to the list of nodes passed as first argument
    ...
    changeEdgeInfo = function(pairInfo, index, edgesInfo, leftDeco, rightDeco, debug){
      if (private$debug){
    ...
    # get the set of edges with a certain pattern indecorations
    ...
    updateGraph = function(){
      ...
      for(i in 1:nrow(private$edgesInfo)){
        ...
        if (edge$left == "arrow" & edge$right == "none"){
          ...
          private$graph <- set.arc(private$graph,from=edge$to,to=edge$from,check.cycles=FALSE,debug=FALSE)
        }
        else{
          if (edge$right == "arrow" & edge$left == "none"){
            # sets the orientation in the graph
            private$graph <- set.arc(private$graph,from=edge$from,to=edge$to,check.cycles=FALSE,debug=FALSE)
          }
        }
      }
    },
    ...
    getModelVStructuresDataFrame = function(){
      if (private$debug){
      ...
      if (private$containsData(candidatePairsdf)){
        ...
        # selects the pattern none - arrow - none -node
        vs <- private$selectPairPattern(candidatePairsdf, "none","arrow", 
                                          "none", "none")
    ...
    find.row = function(df, row){
      ...
      if (nrow(df) != 0){
        joined <- rbind(row,df)
        found <- (anyDuplicated(rbind(row,df)) > 1)
      }
      return(found)
    },
    ...
    compareVStructures = function(targetVsdf,currentVsdf){
      ...
      if (private$debug){
        cat("Comparing current against target: \n")
      }
      ...
      for(i in 1:nrow(currentVsdf)){
        ...
        if (!private$find.row(targetVsdf,vs) & !private$find.row(targetVsdf,vsInverted)){
          fn <- fn+1
          if (private$debug){
            cat("V-structure not found on target structure: ",fn,"\n")
          }
        }
      }
...
# easy constructor
buildObject <- function(net, data, pc=FALSE, alpha=0.05, debug=FALSE){
  MampcSearch$new(net, data, pc, alpha, debug)
}
